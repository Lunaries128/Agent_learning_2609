...
def make_chart(csv_path: str, chart_type: str, x: str, y: str = "") -> str:
    """画图并保存 PNG。chart_type 只能是 line(折线)/bar(柱状)/scatter(散点)/hist(直方图)。
    hist 不需要 y。x、y 是 CSV 的列名。返回保存路径。"""
    # ---------- 安全护栏①：图表类型白名单 ----------
    # 模型偶尔会发明 "pie"（饼图）之类我们不支持的类型，
    # 白名单检查保证只处理四种已知图形，其余直接拒绝并告知可选值
    if chart_type not in ("line", "bar", "scatter", "hist"):
        return "错误：chart_type 必须是 line/bar/scatter/hist"

    df = pd.read_csv(csv_path)
    # ---------- 安全护栏②：列名存在性校验 ----------
    # 条件拆解：x 不存在，或者（不是 hist 图且 y 不存在）-> 拒绝
    # hist 直方图不需要 y，所以对它豁免
    if x not in df.columns or (chart_type != "hist" and y not in df.columns):
        return f"错误：列不存在，可用列 {list(df.columns)}"

    # 创建画纸(fig)和坐标系(ax)；figsize=(8,5) 是 8英寸x5英寸
    ...
    path = os.path.join("charts", f"chart_{chart_type}_{x}.png")
